...
const DashboardSidebar = ({ profile }: { profile: UserInfo | null }) => {
  ...
  // SEMENTARA MATIKAN DULU, INI UNTUK FILTER AKSES BERDASARKAN ROLE
                ...
                <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild>
                    <NavLink
                      to={item.url}
                      className={`${
                        pathname === item.url ? "bg-primary text-muted" : ""
                      }`}
                    >
                      <item.icon
                        className={`h-4 w-4 ${
                          pathname === item.url ? "text-muted" : ""
                        }`}
                      />
