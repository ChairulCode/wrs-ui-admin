...
import { PrestasiDetail } from "@/types/data";
...
const PrestasiDetailPage = () => {
	...
	const [data, setData] = useState<PrestasiDetail | null>(null);
	...
	useEffect(() => {
		const getData = async () => {
			try {
				const response = await getRequest(`prestasi/${prestasi_id}`);
	...
	if (!data) {
		return (
			<>
				<div className='prestasi-not-found'>
					<div className='not-found-content'>
						<div className='not-found-icon'>📚</div>
						<h1>Prestasi tidak ditemukan</h1>
						<p>Maaf, prestasi yang Anda cari tidak tersedia</p>
