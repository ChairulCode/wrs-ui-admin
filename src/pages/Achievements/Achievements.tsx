...
const Achievements = () => {
	...
	const [achievementsBackup, setAchievementsBackup] = useState<ApiResponse<Prestasi>["data"] | []>([]);
	const [achievementsFiltered, setAchievementsFiltered] = useState<ApiResponse<Prestasi>["data"] | []>([]);
		...
		try {
			if (editingId) {
				...
				if (gambar) {
					...
					const uploadFoto = await postRequest("/galleries/add/achievements", foto);
				...
				const uploadAchievementData = await postRequest("/prestasi", dataToSubmit);
				if (gambar) {
					const foto = new FormData();
					foto.append("image", gambar);
					...
					<CardContent>
						<Table>
							...
							<TableBody>
									...
									achievementsFiltered.map((achievement, index) => (
										<TableRow key={achievement.prestasi_id}>
											...
											<TableCell className='text-right flex gap-2'>
												...
												<Button size='sm' variant='destructive' onClick={() => popupDelete(achievement.prestasi_id || "")}>
								...
								{!isLoading && achievementsFiltered && achievementsFiltered.length === 0 && (
									<TableRow>
