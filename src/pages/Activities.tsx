...
const initialFormData: Activites = {
	judul: "",
	ringkasan: "",
	konten_lengkap: "",
	gambar_utama: "",
...
const Activites = () => {
	const [activites, setAcitivites] = useState<Activites[]>([]);
	const [achievementsFiltered, setAcitivitesFiltered] = useState<Activites[]>([]);
	const [loading, setLoading] = useState(false);
	const [open, setOpen] = useState(false);
	const [editingId, setEditingId] = useState<string | null>(null);
	const [formData, setFormData] = useState<Activites>(initialFormData);
	...
	const fetchAcitivites = async () => {
		...
		try {
			...
			const res = await getRequest(`/kegiatan?${queryParams.toString()}`);
			const responseData: AchievementResponse = res;

			setAcitivites(responseData.data || []);
			setAcitivitesFiltered(responseData.data || []);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			if (editingId) {
				...
				const res = await putRequest(`/kegiatan/${editingId}`, dataToSubmit);
				...
				const res = await postRequest("/kegiatan", dataToSubmit);
	...
	const executeDelete = async (id: string) => {
		...
		try {
			...
			const isLastItemOnPage = Activites.length === 1;
	...
	const openEditDialog = (achievement: Activites) => {
	...
	useEffect(() => {
		setAcitivitesFiltered(activites);
		setAcitivitesFiltered((prev) => prev.filter((achievement) => achievement.judul.toLowerCase().includes(searchTerm.toLowerCase())));
		setMetadata((prev) => ({
			...prev,
			currentPage: 1,
		}));
